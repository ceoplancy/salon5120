...
export default function useOnClickOutside({ inRef, exceptRef, handler }: PropsT) {
  useEffect(() => {
    const listener = (event: TouchEvent | MouseEvent) => {
      // Do nothing if clicking ref's element or descendent elements
      if (!inRef?.current || inRef?.current?.contains(event.target as Node) || exceptRef?.current?.contains(event.target as Node)) {
        return;
      }
